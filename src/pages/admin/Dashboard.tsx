...
const StatCard = ({ title, value, change, icon, trend, subtitle, onClick, clickable = false }: StatCardProps) => {
  ...
  if (clickable && onClick) {
    return (
      <Card 
        className="p-8 bg-white rounded-xl shadow-lg border-none text-steel-900 hover:shadow-xl transition-all duration-200 cursor-pointer hover:scale-105 hover:bg-slate-50 relative group"
        onClick={onClick}
      >
        {cardContent}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-slate-800">Loading Admin Dashboard...</h2>
          <p className="text-slate-600 mt-2">Please wait while we load your data</p>
        </div>
      </div>
    );
  }
