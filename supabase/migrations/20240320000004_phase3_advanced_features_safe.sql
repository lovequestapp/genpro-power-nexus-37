-- Phase 3: Advanced Project Management Features (Safe Migration)
-- This migration safely adds milestone management, audit trails, and status workflow
-- Only creates tables if they don't already exist

-- Create milestones table for project progress tracking (if not exists)
...
-- Create project audit trail table (if not exists)
...
-- Create project status workflow rules table (if not exists)
...
-- Create triggers to automatically update updated_at (if not exist)
DO $$
BEGIN
...
-- Insert default status workflow rules (if not exist)
...
-- Create function to calculate project progress from milestones (if not exists)
...
-- Create function to log project changes (if not exists)
...
-- Create trigger to automatically log project changes (if not exists)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'log_project_changes') THEN
...
-- Create function to validate status transitions (if not exists)
...
-- Create trigger to validate status transitions (if not exists)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'validate_project_status_transition') THEN
